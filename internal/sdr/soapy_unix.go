...
func loadSoapy(config Config) (*soapyAPI, error) {
	...
	for _, corePath := range unixSoapyCoreCandidates(config) {
		...
		if config.Driver == "rtlsdr" {
			for _, dep := range unixRTLDependencies() {
		...
		// Packaged SoapySDR installs often register modules from the default
		// plugin path as soon as libSoapySDR is opened.
		config.trace("SoapySDR/%s: using library without an explicit module path", config.Driver)
		return api, nil
	}
	if len(failures) == 0 {
		failures = append(failures, fmt.Errorf("libSoapySDR not found"))
	}
...
func unixSoapyCoreCandidates(config Config) []string {
	...
	names := []string{"libSoapySDR.so.0.8", "libSoapySDR.so.0", "libSoapySDR.so", "libSoapySDR.dylib"}
	var out []string
	if root != "" {
...
func unixRTLDependencies() []string {
	names := []string{"libusb-1.0.so.0", "librtlsdr.so.0", "libusb-1.0.dylib", "librtlsdr.dylib"}
	if homebrew := unixHomebrewPrefix(); homebrew != "" {
		names = append(names,
